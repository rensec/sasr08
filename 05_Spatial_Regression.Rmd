...
```{r include=T, echo=T, warning=F, message=FALSE}
...
First we try to figure out whether we need a spatial model in the first place.
For this purpose, we use the Moran's I test.
...
We should not use a linear regression, we should use a spatial model.

### Lagrange multiplier tests for spatial lag and spatial error dependencies

Now we need to figure out which type of spatial models we should use.
For this purpose, we use Lagrange multiplier tests for spatial lag and spatial error dependencies.

```{r }
lm.LMtests(olsreg, listw, test=c("LMlag", "LMerr"))
```

We get an interesting case where we reject both hypotheses.
It appears that we should use both a spatial lag and a spatial error model.
So the only way to decide which model to use is now to base your modelling on your theory, or to test them all and see whether the results differ.

### Spatial lag model

We start with the most used model: the spatial lag model.
...
Note that we explicitly specify that this is not a Durbin model.
Just like when using the linear regression model, we find a significantly negative effect of household income and housing values on the crime rate.
Coefficients are very similar to the LRM coefficients.
This is often the case given that LRM coefficients are technically still BLUE.
...
So which model do we choose? Check your theory!
